// 向给定ID的用户发起请求
axios.get('/NanhaiSea/js/serve.js')
  .then(function (response) {
    ...
    function initPage() {
      ...
      for (var index = 0; index < data.list.length / 10; index++) {
        var oSpan = document.createElement('span');
        oSpan.innerHTML = (index);

        oSpan.onclick = function () {
          let item = data.list.slice(this.innerHTML * 10, (+this.innerHTML + 1) * 10);
          dataTreating(item);
          console.log(1);
        }
        oPage.appendChild(oSpan);
        oSpan.style.border = "1px solid #868686"
      }
    }
    initPage()
  })
